...
glance(final_model) %>% 
  select(AIC) %>%
  kable(digit = 3, caption = "Final Model")
```

```{r}
glance(full_model) %>% 
  select(AIC) %>%
  kable(digit = 3, caption = "Full Model (before backward selection)")
```
