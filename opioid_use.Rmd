...
op_users <- us19 %>%
  filter(OP_USE == 1) %>%
  select(DEM_AGE10, DEM_REGION, DEM_GENDER, DEM_INCOME, DEM_EDU, OP_NMUYR, DAST_SUM, all_of(keep), starts_with("MENT"), starts_with("DEM_RACE"), PAIN_ACUTEYR, PAIN_CHRONICYR)
...
op_users <- op_users %>%
  ...
  select(-DEM_RACE_AIAN, -DEM_RACE_ASIAN, -DEM_RACE_BLACK, -DEM_RACE_OTH, -DEM_RACE_WHITE, -DEM_RACE_NHPI) %>%
